  ...
  return (
    <Layout>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight font-serif text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.6)]">
            My Reports
          </h1>
          <p className="text-muted-foreground mt-1">
            Manage and view your generated data stories.
          </p>
        </div>
        <Link href="/create">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reports.map((report) => (
          <Card
            key={report.id}
            className="group hover:shadow-lg transition-all duration-300 border-slate-200 hover:border-primary/50 cursor-pointer animate-in fade-in zoom-in-95 duration-300"
          >
            ...
            <CardFooter className="pt-0">
              <Link
                href={`/report/${report.id}?title=${encodeURIComponent(
                  report.title
